...
module.exports.api = {
    ...
    connectAndSubscribe: async (args, devices, onUplinkDevicePortBufferDateLatLng, onDownlinkAckDeviceAcknowledged) => {
        ...
        try {
            ...
            // Confirmed-downlink (n)ack. v4 reports deviceInfo.devEui and fCntDown.
            const handleAck = async (topic, message) => {
                ...
                const id = deveui || (obj.deviceInfo && obj.deviceInfo.devEui);
                if (!id) {
                    console.log("Chirpstack: ack message without a resolvable devEUI, skipping");
                    return;
                }
                await onDownlinkAckDeviceAcknowledged(client, id, obj.acknowledged === true, obj.fCntDown);
            };
